...
- Both the client and server can call `ads_request_exchange` to exchange a list of [AURORA Blob](AURORA%20Blob)s
...
- Client side component for [#Exchanging Data `exchange`](%23Exchanging%20Data%20%60exchange%60) and [#Accepting Exchanges](%23Accepting%20Exchanges)
- Requires:
	- A completed [#Configuration](%23Configuration) struct
	- Local copy of `exchange_data` (from [AURORA Connection Instance - ACI|ACI](AURORA%20Connection%20Instance%20-%20ACI%7CACI) and [AURORA Completion Notification - ACN|ACN](AURORA%20Completion%20Notification%20-%20ACN%7CACN)) 
	- No additional initialization
- Can be called multiple times (does not maintain/use internal context)
- Usage:
	1. Allocate an `exchange_data` structure
	2. Initialize [AURORA Connection Instance - ACI|ACI](AURORA%20Connection%20Instance%20-%20ACI%7CACI) and [AURORA Completion Notification - ACN|ACN](AURORA%20Completion%20Notification%20-%20ACN%7CACN) with the `exchange_data` [AURORA Blob|blobs](AURORA%20Blob%7Cblobs)
	3. Call on `ads_request_exchange`
	4. Connect [AURORA Connection Instance - ACI|ACI](AURORA%20Connection%20Instance%20-%20ACI%7CACI) and [AURORA Completion Notification - ACN|ACN](AURORA%20Completion%20Notification%20-%20ACN%7CACN) with the `exchange_data` [AURORA Blob|blobs](AURORA%20Blob%7Cblobs) returned by the exchange function
...
- Requires:
	- ADS to be initialized ([#Initialization](%23Initialization))
...
- Server side (part 2) component for [#Exchanging Data `request_exchange`](%23Exchanging%20Data%20%60request_exchange%60)
- Can only be called once per socket descriptor
- Requires:
	- ADS to be initialized ([#Initialization](%23Initialization))
	- A Socket File Descriptor returned from [#Accepting Exchanges](%23Accepting%20Exchanges)
- Returns a completed `exchange_data` type, or `NULL` upon failure
...
## Notes
- All exchange data should follow the [AURORA Blob](AURORA%20Blob) format.
