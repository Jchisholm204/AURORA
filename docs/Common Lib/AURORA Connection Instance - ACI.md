...
- Requires:
	- [AURORA Discovery Service - ADS](AURORA%20Discovery%20Service%20-%20ADS.md) (for key-value exchange)
	- UCP

## Initialization
The ACI must be initialized in two stages; [#Instance Creation](#Instance%20Creation) and [#Instance Connection](#Instance%20Connection).
### Instance Creation
...
- Generates the worker connection ID, and places it within the `blob_t`
	...
	- The memory placed within the `blob_t` must also not be freed
		- This memory must get passed to [`aci_connect_instance`](#ACI%20Connection)
		- The Connect Instance function will handle cleanup
- Requires:
	- An empty [AURORA Blob](AURORA%20Blob.md) structure
- Returns: 
	- Unconnected ACI instance
	- Filled out exchange data ([AURORA Blob](AURORA%20Blob.md), return by parameter)
...
- Requires:
	- An existing ACI ([#Instance Creation](#Instance%20Creation))
	- The [AURORA Discovery Service - ADS](AURORA%20Discovery%20Service%20-%20ADS.md) remote exchange to have successfully completed
...
- While ACI destruction is a two part process, [#Instance Destruction](#Instance%20Destruction) will attempt to perform [#Instance Disconnection](#Instance%20Disconnection) before destroying the instance.
...
- This layer should be the only/primary layer used outside of the [Common Lib](Common%20Lib.md)
...
## Extended Interface
- Since this module abstracts the UCP worker and endpoint, any UCP functions that require access to these structures are abstracted through the [#Extended Interface](#Extended%20Interface)
