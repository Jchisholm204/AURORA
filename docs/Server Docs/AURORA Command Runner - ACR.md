...
- Each thread is granted a thread context, containing the [AURORA Instance Manager - AIM|AIM](AURORA%20Instance%20Manager%20-%20AIM%7CAIM) instance and an entry.
...
- [#ACR](%23ACR) threads are also given a "`flags`" parameter that can be used on a per command basis
...
## Initialization
- Requires:
	- [AURORA Instance Manager - AIM||AIM](AURORA%20Instance%20Manager%20-%20AIM%7C%7CAIM) (Connected)
...
- Options for the `ACR_cmd_fn` argument within [#Dispatch Command Thread](%23Dispatch%20Command%20Thread)
...
- Simply returns the [AURORA Instance Manager - AIM|AIM](AURORA%20Instance%20Manager%20-%20AIM%7CAIM) entry back to the queue
#### Checkpoint
...
- Precursor to [#Connection Down](%23Connection%20Down)
- Flags = socket opened by [AURORA Connection Listener - ACL](AURORA%20Connection%20Listener%20-%20ACL)
...
- Use [#Context Release Retry](%23Context%20Release%20Retry) for more consistent results
```c
eACR_error _acr_ctx_release(struct aurora_command_ctx *pCtx);
```
### Context Release Retry
- [#Context Release](%23Context%20Release), but with an auto retry mechanism to ensure the release follows through
